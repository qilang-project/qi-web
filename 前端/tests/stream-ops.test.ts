...
const li = (k: string, text: string) => `<li data-key="${k}">${text}</li>`;

function container(withIgnore = true): HTMLElement {
  document.body.innerHTML =
    `<ul id="msgs"${withIgnore ? ' data-ignore' : ''}></ul>`;
  return document.getElementById('msgs')!;
}

function apply(ops: StreamOp[], id = 'msgs'): void {
...
describe('追加', () => {
  ...
  it('按顺序追到末尾', () => {
    apply([['a', 'm1', li('m1', '一')], ['a', 'm2', li('m2', '二')]]);
  ...
  it('键已存在就原地替换 —— 这也是「更新某一条」的做法', () => {
    apply([['a', 'm1', li('m1', '一')], ['a', 'm2', li('m2', '二')]]);
    apply([['a', 'm1', li('m1', '改过的一')]]);
    expect(keys()).toEqual(['m1', 'm2']);      // 位置不动
    expect(document.querySelector('[data-key="m1"]')!.textContent).toBe('改过的一');
  });

  it('服务端漏写 data-key 时自动补上，否则下次就找不到它了', () => {
    apply([['a', 'm9', '<li>没写键</li>']]);
    expect(keys()).toEqual(['m9']);
    apply([['a', 'm9', '<li>换一条</li>']]);
    expect(keys()).toEqual(['m9']);            // 是替换，不是又加一条
  });
});
...
describe('前插', () => {
  ...
  it('插到最前面（新消息在最上面的时间线）', () => {
    apply([['a', 'm1', li('m1', '一')], ['p', 'm0', li('m0', '零')]]);
    expect(keys()).toEqual(['m0', 'm1']);
  });

  it('已存在的键前插 = 移到最前并替换', () => {
    apply([['a', 'm1', li('m1', '一')], ['a', 'm2', li('m2', '二')]]);
    apply([['p', 'm2', li('m2', '二改了')]]);
    expect(keys()).toEqual(['m2', 'm1']);
...
describe('删除与清空', () => {
  ...
  it('按键删一项，其他不动', () => {
    apply([['a', 'm1', li('m1', '一')], ['a', 'm2', li('m2', '二')], ['a', 'm3', li('m3', '三')]]);
    apply([['d', 'm2']]);
    expect(keys()).toEqual(['m1', 'm3']);
  });
...
describe('稳健性', () => {
  it('容器还没渲染出来时整批丢掉，不抛', () => {
    document.body.innerHTML = '';
    expect(() => apply([['a', 'm1', li('m1', '一')]], '还不存在')).not.toThrow();
  });

  it('只找直接子节点：嵌套列表里同名的键不该被误伤', () => {
    document.body.innerHTML =
      '<ul id="msgs" data-ignore><li data-key="x"><ul><li data-key="m1">里面的</li></ul></li></ul>';
    apply([['a', 'm1', li('m1', '外面的')]]);
    expect(keys()).toEqual(['x', 'm1']);       // 新增一条，没有动嵌套里那条
  ...
  it('多个容器一帧里各干各的', () => {
    ...
    applyStreams([
      { c: 'msgs', ops: [['a', 'm1', li('m1', '消息')]] },
      { c: 'logs', ops: [['a', 'l1', li('l1', '日志')]] },
    ]);
